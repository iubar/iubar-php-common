...
class WebUtil {
	...
	public static function check_brokenlink($pageToCheck) {
		...
		if (@$domDoc->loadHTMLFile($pageToCheck)) {
			//note that errors are suppressed so DOMDocument doesn't complain about XHTML
					...
					if ($attributeName == 'href') {
						...
						if ($attributeName == 'href') {
							...
							if (in_array($returnCode, $badStatusCodes)) {
								$badLinks[] = ['name' => $currLink->nodeValue, 'link' => $attributeValue->value];
							} elseif ($finalURL != $attributeValue->value) {
								$changedLinks[] = [
									'name' => $currLink->nodeValue,
									'link' => $attributeValue->value,
									'newLink' => $finalURL
								];
							} else {
								$goodLinks[] = ['name' => $currLink->nodeValue, 'link' => $attributeValue->value];
							}
						}
					}
				}
